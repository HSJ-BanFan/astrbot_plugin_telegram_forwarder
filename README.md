...
* **file_direct_link_base_url**: 非 APK 普通文件发送失败时的直链基地址，例如 `.zip` 被 QQ 富媒体上传拒收后会优先改发 `基地址 + 文件名`；留空时仅在失败疑似路径问题（如 QQ 端 ENOENT）且源路径与映射路径不同的情况下，才用源文件路径重发一次；上传被拒（rich media transfer failed）时不再原样重试，消息留在队列等待下轮重试。
