...
* **🛠️ 高级控制逻辑**
  * **灵活过滤**: 内置关键词 / 正则**黑名单**过滤（命中即丢弃）；可用负向正则实现「只转发含关键词」。
  * **监听加速**: `monitor_keywords` / `monitor_regex` 仅用于命中后**立即转发**，不会过滤掉未命中的消息。
...
| `filter_keywords` | `list` | `[]` | **黑名单**关键词：包含这些词的消息将被丢弃（不是「只转发这些词」）。 |
| `filter_regex` | `string` | `""` | **黑名单** Python 正则：`re.search` 命中即丢弃。可用负向正则做白名单，见下方 FAQ。 |
...
| `monitor_keywords` | `list` | `[]` | **仅加速**：命中后立即触发转发；**不会**过滤掉未命中的消息。 |
| `monitor_regex` | `string` | `""` | **仅加速**：命中后立即触发转发；**不会**过滤掉未命中的消息。 |
...
| `filter_keywords` | `list` | `[]` | 全局**黑名单**关键词：命中即丢弃。 |
| `filter_regex` | `string` | `""` | 全局**黑名单**正则：命中即丢弃。白名单写法见 FAQ。 |
| `monitor_keywords` | `list` | `[]` | 全局监听关键词（命中后立即发送；**不负责过滤**）。 |
| `monitor_regex` | `string` | `""` | 全局监听正则（命中后立即发送；**不负责过滤**）。 |
...
* **Q: 如何设置「只转发包含关键词的消息，其余全部丢弃」？**
  * **A**: 当前**没有**单独的「白名单关键词」开关。相关配置语义如下：
    | 配置项 | 真实作用 | 常见误解 |
    | :--- | :--- | :--- |
    | `monitor_keywords` / `monitor_regex` | 命中后**立即转发**（加速），未命中的消息仍会按 `send_interval` 正常转发 | ❌ 误当成「只监控这些词」 |
    | `filter_keywords` / `filter_regex` | **黑名单**：命中后**丢弃** | ❌ 误当成「只保留这些词」 |

    **推荐做法：用 `filter_regex` 写负向正则（命中 = 丢弃）**，把「不包含关键词」的消息全部过滤掉。

    示例：只想转发正文里出现 `监听` 或 `更新` 的消息，其余全部丢弃——在**全局**或**该频道**的 `filter_regex` 填入：
    ...
    说明：
    1. 把 `监听|更新` 换成你的关键词，多个用 `|` 分隔。
    2. 实际过滤路径对正则使用**忽略大小写**（`re.IGNORECASE`）。
    3. 纯媒体、无文本消息的匹配文本为空，负向正则会将其**丢弃**（符合「只保留含关键词」）。
    4. 若同时需要「命中关键词时立刻发、不命中的直接丢」，可**同时**配置：
       - `monitor_keywords: ["监听", "更新"]`（加速命中消息）
       - `filter_regex: ^(?!.*(?:监听|更新)).*$`（丢掉未命中消息）
    5. 配置后看日志：应出现 `[Filter] 消息 xxx 命中正则匹配: ...`，未命中消息会从队列移除且不转发。
* **Q: 音频链接不显示？**
